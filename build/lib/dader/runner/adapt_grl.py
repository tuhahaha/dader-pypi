...
def adapt_grl(encoder, matcher, alignment, 
    ...
    optimizer = optim.Adam(list(encoder.parameters())+list(matcher.parameters())+list(alignment.parameters()), lr=lr)
    ...
    for epoch in range(epochs):
        ...
        if len(src_data_loader)>len(tgt_data_loader):
            data_zip = enumerate(itertools.zip_longest(src_data_loader, tgt_data_loader))
        else:
        ...
        for step, (src, tgt) in data_zip:
            if tgt:
                ...
                dom_preds = alignment(feat_concat, alpha=alpha)
                cls_loss = CELoss(preds, labels)
                label_src = torch.ones(feat_src.size(0)).to(device).unsqueeze(1)
                label_tgt = torch.zeros(seq_tgt.size(0)).to(device).unsqueeze(1)
                label_concat = torch.cat((label_src, label_tgt), 0).float()
                loss_dom = BCELoss(dom_preds, label_concat)
                
                loss = alpha * cls_loss + beta * loss_dom
            else:
